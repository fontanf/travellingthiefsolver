...
using CityState = travellingthiefsolver::packing_while_travelling::CityState;
using CityStateId = travellingthiefsolver::packing_while_travelling::CityStateId;
    ...
    cities_(instance.number_of_cities())
{
    ...
    for (CityPos city_pos = 1;
        ...
        Time time = instance.duration(distances, city_id_prev, city_id, visit_prev.weight_from_start);
    ...
    travel_time_ += instance.duration(distances, city_id_prev, 0, weight_);
...
public:
            ...
            std::mt19937_64& generator)
    {
        ...
        for (const auto& tsp_solution: tsp_solutions) {
            ...
            travellingthiefsolver::packing_while_travelling::EfficientLocalSearchParameters els_parameters;
            els_parameters.verbosity_level = 0;
            auto els_output = travellingthiefsolver::packing_while_travelling::efficient_local_search(
                    pwt_instance,
                    els_parameters);
            ...
            algorithm_formatter_.update_solution(ls_solution.to_solution(distances_), "");
...
template <typename Distances>
EfficientLocalScheme<Distances>::EfficientLocalScheme(
        const Instance& instance,
        const Distances& distances,
        const std::vector<std::vector<CityState>>& city_states,
        const EfficientLocalSearchParameters& parameters,
...
template <typename Distances>
void EfficientLocalScheme<Distances>::local_search(
        Solution& solution,
        std::mt19937_64& generator,
        Perturbation)
{
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::EvaluateChangeCityStateOutput
EfficientLocalScheme<Distances>::evaluate_change_city_state_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id,
        CityStateId city_state_id)
{
...
template <typename Distances>
Profit EfficientLocalScheme<Distances>::evaluate_two_opt_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id_1,
        CityId city_id_2)
{
    ...
    remaining_distance -= solution.visit(city_pos_1).distance_from_start;
    {
        ...
        CityId city_id_4 = (city_pos_2 == instance_.number_of_cities() - 1)?
            ...
            solution.visit(city_pos_2 + 1).city_id;
        // Two-opt 1 - A => 1 - 2.
        remaining_distance -= distances_.distance(city_id_1, city_id_3);
        remaining_distance += distances_.distance(city_id_1, city_id_2);
        // Two-opt 2 - Y => A - Y.
        remaining_distance -= distances_.distance(city_id_2, city_id_4);
        remaining_distance += distances_.distance(city_id_3, city_id_4);
    }
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::EvaluateTwoOptChangeCityStatesOutput
EfficientLocalScheme<Distances>::evaluate_two_opt_change_city_states_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id_1,
        CityId city_id_2)
{
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::EvaluateShiftOutput
EfficientLocalScheme<Distances>::evaluate_shift_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id)
{
    ...
    for (CityPos city_pos = 1;
        ...
        time_from_start.push_back(time_from_start.back() + instance_.duration(
                    distances_,
                    city_id_prev,
                    city_id_cur,
                    weight_from_start.back()));
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::EvaluateShiftChangeCityStateOutput
EfficientLocalScheme<Distances>::evaluate_shift_change_city_state_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id,
        CityStateId city_state_id)
{
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::EvaluateShiftChangeCityState2Output
EfficientLocalScheme<Distances>::evaluate_shift_change_city_state_2_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id)
{
    const CityState& city_state_old = city_states_[city_id][solution.city(city_id).city_state_id];
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::EvaluateChangeTwoCityStatesOutput
EfficientLocalScheme<Distances>::evaluate_change_two_city_states_move(
        const EfficientLocalSearchSolution& solution,
        CityId city_id_1)
{
    CityPos city_pos_1 = solution.city(city_id_1).position;
...
template <typename Distances>
void EfficientLocalScheme<Distances>::apply_perturbation(
        Solution& solution,
        const Perturbation&,
        std::mt19937_64& generator) const
{
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::Solution
EfficientLocalScheme<Distances>::cycle_crossover(
        const Solution& solution_parent_1,
        const Solution& solution_parent_2,
        std::mt19937_64& generator)
{
...
template <typename Distances>
typename EfficientLocalScheme<Distances>::Solution
EfficientLocalScheme<Distances>::crossover(
        const Solution& solution_parent_1,
        const Solution& solution_parent_2,
        std::mt19937_64& generator)
{
...
template <typename Distances>
ItemId EfficientLocalScheme<Distances>::distance(
        const Solution& solution_1,
        const Solution& solution_2) const
{
    CityPos d = 0;
        ...
        const EfficientLocalSearchParameters& parameters)
{
    ...
    EfficientLocalScheme<Distances> local_scheme(
            instance,
            distances,
            city_states,
            parameters,
    ...
    algorithm_formatter.update_solution(solution.to_solution(distances), "");
        ...
        const EfficientLocalSearchParameters& parameters)
{
    ...
    localsearchsolver::GeneticLocalSearchParameters<EfficientLocalScheme<Distances>> ls_parameters;
